...
- **Dependencies:** TIP-24. **Priority:** P0 demo correctness.
- **Task:** remove the stale HTML `hidden` state from the SVG detection overlay whenever 2D detection is active; extend E2E from DOM-node counting to real computed visibility.
- **Acceptance:** after mock detections arrive in RGB/Depth, lock rectangles exist and the SVG is not hidden, is displayed and has a non-zero camera-sized layout box.
- **Detail:** `docs/TIP-25-DETECTION-OVERLAY-VISIBILITY.md`.
