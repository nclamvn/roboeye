...
// P1-B-2 metric: Depth Pro chạy trên khung đông cứng
...
function exportAnnotations(fmt: 'coco' | 'yolo' | '3d' | 'kitti') {
  ...
  if (fmt === 'kitti') {
    if (!metricBoxes.some(Boolean)) return; // KITTI cần metric mét thật
  ...
  if (fmt === 'yolo') {
    ...
    // 3D JSON: nếu có metric (Depth Pro) thì xuất mét thật, không thì view-space tương đối
    ...
    const out = {
      note: 'RoboEye 3D annotations. box3d ở không gian view, tỷ lệ TƯƠNG ĐỐI (chưa metric). Bật Metric mode (Depth Pro) để ra mét thật.',
      scale: 'relative',
      image: { width: W, height: H },
      objects: lastBoxes.map((b, i) => ({
        label: b.label,
...
function spawnMetricWorker() {
  metricWorker = new Worker(new URL('./worker/metric-worker.ts', import.meta.url), { type: 'module' });
  metricWorker.onmessage = (ev: MessageEvent) => {
    const m = ev.data;
    if (m.type === 'loading') shell.setMetricStatus('đang tải Depth Pro (~600MB)…');
    else if (m.type === 'ready') shell.setMetricStatus('Depth Pro sẵn sàng, nhấn F để đo');
      ...
      shell.setMetricStatus('metric lỗi');
    }
  };
  metricWorker.postMessage({ type: 'init', forceWasm, localModels });
}
...
function computeMetricOnFreeze() {
  ...
  detW = img.width;
  detH = img.height;
  shell.setMetricStatus('đang đo Depth Pro…');
...
const shell = createShell({
  ...
  onFreeze: (f) => {
    ...
    if (f) computeMetricOnFreeze();
      ...
      metricBoxes = [];
      if (metricMode) shell.setMetricStatus('nhấn F trên khung để đo mét');
    }
  },
  onMetric: (on) => {
    metricMode = on;
